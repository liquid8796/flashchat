...
class SettingsScreenState extends State<SettingsScreen> {
      ...
      if (value.error == null) {
        ...
        storageTaskSnapshot.ref.getDownloadURL().then((downloadUrl) {
          ...
          Firestore.instance
            ...
            final upSuccess = SnackBar(
              content: Text('Uploaded!'),
              duration: Duration(seconds: 1),
            );
            Scaffold.of(context).showSnackBar(upSuccess);
          }).catchError((err) {
            setState(() {
              isLoading = false;
            });
            ...
            Scaffold.of(context).showSnackBar(errorToast);
          });
        }, onError: (err) {
          setState(() {
            isLoading = false;
          });
          final wrongImage = SnackBar(
            content: Text('This file is not an image!'),
            duration: Duration(seconds: 1),
          );
          Scaffold.of(context).showSnackBar(wrongImage);
        });
      } else {
        setState(() {
          isLoading = false;
  ...
  void handleUpdateData() {
    ...
    Firestore.instance
      ...
      Scaffold.of(context).showSnackBar(successToast);
    }).catchError((err) {
      setState(() {
        isLoading = false;
      });
